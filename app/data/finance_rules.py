...
# ───────────────── 재산세(주택) — 지방세법 룰 ─────────────────
# 출처: 지방세법 제110·111조, 제111조의2(1세대 1주택 특례세율), 지방세법 시행령(공정시장가액비율).
# ⚠️ 세부담 상한(직전연도 대비)·감면·다주택 등은 개별 사안 → 계산에 반영하지 않고 안내만 한다.
PROPERTY_TAX = {
    "as_of": "2024년 이후 기준(연도별 공정시장가액비율 고시 확인 필요)",
    "source_url": "https://www.law.go.kr/법령/지방세법",
    # 공정시장가액비율: 주택 60%. 1세대 1주택은 특례(공시가 구간별 43~45%).
    "fair_ratio_default": 0.60,
    "fair_ratio_one_house": [   # [공시가격 상한(만원), 비율] — 초과분은 마지막 값
        [30000, 0.43], [60000, 0.44], [10**9, 0.45],
    ],
    # 표준세율: [과세표준 상한(만원), 누진공제 방식용 (기본세액, 초과세율)]
    "rate_standard": [
        [6000, 0, 0.001],
        [15000, 6, 0.0015],
        [30000, 19.5, 0.0025],
    ...
    "one_house_cap": 90000,        # 특례세율 적용 공시가격 상한(9억)
    "edu_tax_ratio": 0.20,          # 지방교육세 = 재산세액의 20%
    "city_area_ratio": 0.0014,      # 도시지역분 = 과세표준의 0.14%(해당 지역)
    "note": ("세부담 상한·감면·다주택 중과는 반영하지 않은 참고 계산입니다. "
             "실제 고지세액은 지자체 산정 기준으로 달라질 수 있어요."),
}

# ───────────────── 전세자금대출 — 은행 일반 요건(참고) ─────────────────
# 정책(버팀목·신생아 특례)은 POLICY_PRODUCTS 로 매칭. 여기는 은행 전세대출의 통상 구조.
RENT_LOAN = {
    "as_of": "2025년 통상 기준(은행·보증기관별 상이)",
    "ltv_typical": 0.80,            # 보증금 대비 통상 한도 비율(보증기관·상품별 상이)
    "note": ("전세대출 한도는 보증기관(HUG·HF·SGI) 보증한도와 소득·신용에 따라 달라집니다. "
             "이자만 납부(만기일시)가 일반적이며, 실제 조건은 은행 심사로 확정됩니다."),
}
