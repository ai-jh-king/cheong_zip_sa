...
migrations/              Alembic (env.py가 앱 설정 URL 사용) — **head 0024**
...
> **스키마 규칙**: 신규 컬럼은 ①모델에 추가 ②Alembic 마이그레이션 파일 생성(운영 PG) — SQLite 는 `_ensure_columns`(모델 자동 도출)가 시작 시 보강. 'no such column'은 대개 마이그레이션 미적용 or 운영이 SQLite. **마이그레이션 head = 0024**(…0023_name_whitespace → 0024_complex_dong). 스키마 변경은 반드시 Alembic(아래 3).
...
- `skills/deploy-and-db.md` — SQLite↔PostgreSQL·Alembic(0001~0024, head 0024_complex_dong)·**웹푸시·모니터링·백업·doctor**·레이트리밋
