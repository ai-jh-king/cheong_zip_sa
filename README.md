...
- **지도/히트맵**: 구 단위 평단가 마커(좌표 없이도 표시) + 단지 단위(지오코딩 시) 토글. **전체화면 지도**(필터·요약 오버레이), 총액(억) 말풍선 마커·원형 클러스터.

### 청주 특화 (Cheongju-specific)
- **🧭 전입자 온보딩(베타)**: '청주가 처음이세요?' — 직장(SK하이닉스·오송·오창 등) 선택 → 근처 단지 + 시세 + 예산 차액을 큐레이션. *중개·광고 수익이 없어 특정 매물을 권유하지 않습니다.*
- **🏗 개발 호재**: 홈 "청주는 지금" 카드 + 지도 호재 핀. SK하이닉스 P&T7·오창 방사광가속기·테크노폴리스·북청주역세권·OSCO 등 **출처 있는** 실제 개발 이슈(투자 판단 아님 고지). `scripts/seed_landmarks`로 적재.
- **🏭 직주근접(직장·거점 거리)**: 단지에서 SK하이닉스 청주캠퍼스·오창·오송·오송역·도청 등 청주 주요 거점까지 **직선거리**. 청주 실수요(산단 근로자) 맞춤. `scripts/seed_commute`로 적재.
- **🧸 육아 환경 & 초품아**: 단지 반경 어린이집·유치원·학원·소아과 개수 요약 + 초등학교 도보권(초품아) 배지. *시설 데이터 적재 시 활성.*

- **💬 호가 검증**: 어디서 본 매물 가격을 넣으면 그 단지 실거래 분포에서의 위치(중앙값 대비 %·백분위)를 알려줌. **📉 낮은 가격 거래 포착**(급매 레이더)·**분양 탭 구 시세 맥락**도 제공 — 판정 대신 사실+면책.
- **📊 전세가율 신호**: 단지의 전세가율(전세/매매 중앙값)을 '갭 크기·주의점'으로 해석해 표시. 청주의 갭투자·역전세 관심에 대응(수치·근거·면책 — 가격 예측 아님).
...
### 사용자 기능
- **🏠 단지 주민 소통**: 게시판에서 @단지 태그 글에, 작성자의 '우리집'과 일치하면 **주민 뱃지**(서버 대조·자가 등록 기반 명시). 단지 상세에 '단지 이야기' 섹션.
- **우리집(내 아파트)**: 내 단지를 등록하면 홈 최상단에 **최근 매매가+변동%**를 바로 표시. 로그인 시 기기 간 동기화(비로그인은 로컬 저장). 더보기 탭은 **마이페이지**(프로필·우리집·바로가기·집찾기 도구).
- **개인화**: 관심 단지/지역(**관심 단지에도 시세·변동% 표시**), 최근 본 단지, 저장 검색, 내 동네, 맞춤 추천, 대출 프로필. 익명 `device_id`로 시작해 **로그인 시 기기 데이터 자동 승계(union)**.
