...
## v1.285 (2026-08-29) — 집사 소식 재구성: '청주는 지금' + 뉴스 | 도감 | 정보 3탭
- 탭 최상단에 **큰 제목 '청주는 지금'**(로켓 아이콘) 고정, 그 아래 서브탭을 **뉴스 · 도감 · 정보** 3개로 재편(사용자 확정).
  - **뉴스**: 최근 7일 청주·부동산 뉴스만(원문 링크). **정보**: 개발 이슈만 — 추진·계획·확정 배지·기대연도·출처. **도감**: 기존 콘텐츠(시리즈→편).
  - 행 구조·배지 열·더보기 방식은 통합 시절(v1.271)과 동일 유지("구조는 똑같이") — `CityNow`에 `only` 모드(news|info) 추가로 분리.
  - 탭별 안내문·빈 상태·면책 문구도 성격에 맞게 분리(뉴스=저작권 고지 / 정보=변동 가능·보장 없음).
- 기본 진입 탭을 **뉴스**로 변경(첫 탭 일치). 홈 뉴스 배너→뉴스 탭, 소통 도감 카드→도감 탭 딥링크는 각각 섹션을 명시 지정하므로 그대로 동작.
- 검증: verify_all · 빌드 · pytest 223 · 실브라우저(제목·3탭·탭별 내용 분리·무스크롤).
- ※ 로컬 PostgreSQL이 자체 정지(autovacuum 워커 반복 중단→신규 연결 타임아웃) 상태여서 UI 검증은 SQLite 스냅샷으로 수행. 운영 DB와 무관.
## v1.284 (2026-08-09) — 배포 전 전수 감사 + 단위 통일 마감(구입 플랜·예산 찾기 = 억)
- **배포 전 감사(사용자 지시: 오버·구라 점검)**: ①과장·보장 표현 전수 grep — 0건(면책 7곳 확인) ②신규 사용자 시나리오 E2E — 첫 진입(넛지·무스크롤)→구입 플랜 완주(동의 게이트→단지→계산)→지도→검색→상세→청약→소식→소통→더보기, 전 구간 크래시 0 ③계산 정합 실측 — 10억·현금 3억: LTV 70%=**7.00억 한도**·총 필요현금 3.38억(부대비용 3,850만원)·부족분 정직 표기 ④콘솔 에러 0(관리자 토큰 401 무해) ⑤프로드 신선도 — 실거래 8/7 기준·8/8 수집(44,481건)·지오코딩 105% 커버.
- **감사에서 잡은 결함 수정**: 구입 플랜(Loan)·예산 찾기(BudgetPicks)의 매매가·보유현금이 **만원 단위로 남아 있던 것**(v1.278 감사에서 라벨-단위 분리 구조라 누락) → **억 단위**로 통일. 저장(cj_loan_profile)·API는 만원 유지(경계 변환·기존 저장값 호환). 중복 '약 X.XX억' 도우미 제거.
- 검증: verify_all · 빌드 · pytest 223 · E2E 억 입력 계산 완주 실측.
## v1.283 (2026-08-09) — 전세위험 줌 연동·지도 목록 전체·청약 탭 상시 표시
- **전세위험 핀 과밀 처리**(사용자 문의): 도시 전체 뷰(z<14)=**위험 상위 30곳만**(ratio 내림차순 개관) → 확대(z≥14)=화면 범위 안 전부 → 구 선택=그 구 전체. 응답은 1회 캐시 후 줌 이동 시 다시 그림. 신호 배지에 "위험 상위 N곳 — 확대하면 그 지역 전체" 안내.
- **지도 목록 전체 표시**(사용자: 100곳인데 10개만): '더보기' 12개 방식 제거 → 전체(뷰포트 상한 300) 스크롤. 실측 300행.
- **청약 탭 상시 표시**(사용자 확정: 중요 정보): 게이팅 숨김 철회. 미연동 시 탭 안에서 정직 안내 — **"실제 공고 연동 준비 중" 행동 카드**(예시임을 명시 + 청약홈 공식 링크 버튼). 연동되면 카드가 사라지고 실데이터 자동 표시(로컬 실측: live=True 시 카드 미표시·실공고 렌더).
- 검증: verify_all · 빌드 · pytest 223 · 실브라우저(탭 표시·행동카드 조건·목록 300행).
## v1.282 (2026-08-09) — 급상승 화살표 아이콘 + 계약전확인 주제별 접이식
- **거래 급상승 아이콘 = ↑ 화살표**(`trendup` 신설) — 홈 랭킹 카드·검색 첫 화면 동일 적용(기존 불꽃 아이콘 대체).
- **계약 전 꼭 확인: 주제별 접이식**(사용자 확정): 4개 주제(집 문제·가격 적정·보증금 보호·세금/청약)가 **기본 접힘 카드**로 첫 화면에 전부 보임 — 내부 스크롤 0(667px 실측). 주제를 탭하면 그 주제의 링크·'안 보면 위험한 점'이 펼쳐짐(재탭=접힘, N곳 카운트·회전 화살표).
- 검증: verify_all · 빌드 · pytest 223 · 실브라우저(4주제·접힘·스크롤 0·펼침).
## v1.281 (2026-08-09) — 도감 관리자 화면(사장님 직접 운영)
- 지금까지 도감은 repo 의 md 파일 + 시드 재실행 + 배포로만 갱신(= 개발자 경유) → **앱 안에서 직접 관리**:
  - 더보기 하단 '도감 콘텐츠 관리(관리자)' → **ADMIN_TOKEN 인증**(기기에만 저장, 서버는 타이밍세이프 비교) → 목록(미공개 포함·초안 배지·조회수) → 편집기(제목·이모지·순서·공개 토글·**마크다운 본문 + 미리보기**) → 저장 즉시 반영 / 삭제(확인 후).
  - 백엔드: `GET /admin/guides`(전체 목록)·`GET /admin/guides/{id}`(본문 포함) 추가 — 기존 POST/PUT/DELETE 와 동일 인증.
  - 편집기에 왜곡 없음 가이드 명시(수치는 출처와 함께·판정/보장 표현 금지).
- E2E 실측: 진입→인증(401/200)→목록 7편→본문 2,600자 로드→미리보기 렌더.
- 검증: verify_all · 빌드 · pytest 223.
