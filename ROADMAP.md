...
## 📌 현재 상태 스냅샷 (v1.178 · 인수인계용 요약)

> 🔎 2026-07-19: **5축 전면 감사** 완료(해자 신호·전입자 온보딩·주민 커뮤니티/판단 지도·데이터 격차·코드 건전성). 결론=전략 정합성·왜곡없음 준수 **강함/우수**, 실결함 소수만 수정(v1.178). 최대 리스크는 코드가 아니라 **배포 시 시드/키 누락으로 청주특화 기능 ~60% 암전** → 자동 시드 안전망 추가로 창끝(온보딩) 부분 해소, 나머지는 배포 체크리스트·외부 키 과제.
...
## 콘텐츠 시스템 — 집사 도감 (v1.208 ✅)
- [x] 백엔드: `GuideSeries`/`Guide` 모델 + Alembic 0022 + `/guides/*`(시리즈·편·조회수 원자) + `/admin/guides` CRUD(X-Admin-Token) + `scripts/seed_guides`(첫 시리즈 cheongju).
- [x] 프런트: 더보기 진입 카드(우리집 다음) → `GuideBook` 전체화면(시리즈 그리드→편 목록→마크다운 상세) + `renderMd`(XSS-safe React 렌더러: 제목·표·목록·굵게·링크) + 이전/다음 + 공유 + 정체성 고지 + 온보딩 CTA.
- [x] 첫 편 발행: "SK하이닉스 청주캠퍼스로 출퇴근한다면" — 앱 실거래·직주근접 로직 실조회 기반(왜곡 없음, 판정 금지).
- [ ] 후속: 주 1편 발행(관리자 검수), 시리즈 확장(계약/대출/안전/이사/청약), 편별 SEO 랜딩(/g/{id}) 연동.
