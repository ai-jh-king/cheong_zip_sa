...
> ⚠️ **낡음 주의(2026-07-20 기준 v1.189)**: 이 문서는 **v1.176 시점 스냅샷**입니다. **§2(현재 상태)·§3(진행 중 카카오 KOE101·"PostgreSQL 전환 미완")·§4 폴더 경로(`cheongju-realestate`)·§7 zip/`/tmp` 패키징 관례는 모두 낡았습니다**(현재 루트는 `cheong_zip_sa`, PG는 render.yaml에 구성 완료, 패키징은 CLAUDE.md §0대로 zip 불필요). **현재 상태·진행사항은 `ROADMAP.md`·`CHANGELOG.md`를, 검증은 `TESTING.md`를 SSOT로 보세요.** 이 문서에서 유효한 건 **§1 전략(3층 해자)·§8 커뮤니케이션 스타일**뿐입니다.
